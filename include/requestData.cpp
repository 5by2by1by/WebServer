...
// 获取 媒体类型
std::string MimeType::getMime(const std::string &suffix)   // 后缀
{
    if (mime.size() == 0)            //当没有键值对，即里面没有媒体类型
    {
        pthread_mutex_lock(&lock);   // 🔒，添加媒体类型
        if (mime.size() == 0)
        {                                     //mime["key"] = "value"
            mime[".html"] = "text/html";
    ...
    if (mime.find(suffix) == mime.end())    // 找不到对应的后缀，就用“default”表示
        return mime["default"];
    else
        return mime[suffix];     // 能找到对应的后缀
}
...
void requestData::handleRequest()
{
    ...
    while (true)
    {
        ...
        /*开始读取*/
        int read_num = readn(fd, buff, MAX_BUFF);    // 每次读取 n 字节
        /*读取出错则直接退出*/
        if (read_num < 0)
        {
            perror("1");
        ...
        content += now_read;                    // content是每次读取数据的累加。
        ...
        if (state == STATE_RECV_BODY) //post请求的情况
        {
            ...
            {
                content_length = stoi(headers["Content-length"]);     // str to int
            }
            else
            {
                isError = true;
